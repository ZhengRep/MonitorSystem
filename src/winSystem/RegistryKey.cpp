...
RegistryKey::RegistryKey(HKEY rootKey, const TCHAR* entry, bool createIfNotExists, SECURITY_ATTRIBUTES* sa)
{
  initialize(rootKey, entry, createIfNotExists, sa);
}

RegistryKey::RegistryKey(RegistryKey* rootKey, const TCHAR* entry, bool createIfNotExists, SECURITY_ATTRIBUTES* sa)
{
...
void RegistryKey::open(HKEY rootKey, const TCHAR* entry, bool createIfNotExists, SECURITY_ATTRIBUTES* sa)
{
  initialize(rootKey, entry, createIfNotExists, sa);
}

void RegistryKey::open(RegistryKey* rootKey, const TCHAR* entry, bool createIfNotExists, SECURITY_ATTRIBUTES* sa)
{
  initialize(rootKey->m_key, entry, createIfNotExists, sa);
}
...
bool RegistryKey::deleteSubKeyTree(const TCHAR* subkey)
{
  ...
  if (key.getSubKeyNames(NULL, &subkeys2Count) && subkeys2Count != 0) {
    std::vector<StringStorage> subkey2Names(subkeys2Count);
  ...
  if (RegOpenKey(key, subkey, openedKey) != ERROR_SUCCESS) {
    if (createIfNotExists) {
      ...
      if (sa != 0) {
        ...
        ret = RegCreateKeyEx(key, subkey, 0, (LPTSTR)_T(""), 0, KEY_READ | KEY_WRITE, sa, openedKey, &dwDisposition);
      } else {
