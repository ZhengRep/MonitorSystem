...
void ZrleEncoder::splitRectangle(const Rect* rect, std::vector<Rect>* rectList, const FrameBuffer* serverFb, const EncodeOptions* options)
{
  rectList->push_back(*rect);
}

void ZrleEncoder::sendRectangle(const Rect* rect, const FrameBuffer* serverFb, const EncodeOptions* options) throw(IOException)
{
  m_bytesPerPixel = 0;
  m_numberFirstByte = 0;
...
template <class PIXEL_T> void ZrleEncoder::writeRawTile(const Rect* tileRect, const FrameBuffer* fb)
{
  m_oldSize = m_rgbData.size();
...
template <class PIXEL_T> void ZrleEncoder::writePackedPaletteTile(const Rect* tileRect, const FrameBuffer* fb)
{
  int numColors = m_pal.getNumColors();
  m_oldSize = m_rgbData.size();
  UINT8 deltaOffset;
...
void ZrleEncoder::pushRunLengthPaletteRle(int runLength, std::vector<UINT8>* paletteRleData)
{
  do {
    if (runLength > 255) {
...
template <class PIXEL_T> void ZrleEncoder::writePaletteRleTile(const Rect* tileRect, const FrameBuffer* fb)
{
  int numColors = m_pal.getNumColors();
...
template <class PIXEL_T> void ZrleEncoder::writePixelToPlainRleTile(const PIXEL_T px, PIXEL_T* previousPx)
{
...
template <class PIXEL_T> void ZrleEncoder::fillPalette(const Rect* tileRect, const FrameBuffer* fb)
{
  // Clear the palette.
  m_pal.reset();
...
template <class PIXEL_T> void ZrleEncoder::copyPixels(const Rect* rect, const FrameBuffer* fb, UINT8* dst)
{
...
void ZrleEncoder::copyCPixels(const Rect* rect, const FrameBuffer* fb, UINT8* dst)
{
  const int rectHeight = rect->getHeight();
  const int rectWidth = rect->getWidth();
