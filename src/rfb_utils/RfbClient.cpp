...
void RfbClient::sendUpdate(const UpdateContainer* updateContainer, const CursorShape* cursorShape)
{
...
void RfbClient::execute()
{
  ...
  try {
    ...
    RfbCodeRegistrator codeRegtor(&dispatcher, &srvToClCaps, &clToSrvCaps, &encCaps);
    // Init modules
    // UpdateSender initialization
    m_updateSender = new UpdateSender(&codeRegtor, m_desktop, this, &output, m_id, m_desktop, m_log);
    ...
    m_clipboardExchange = new ClipboardExchange(&codeRegtor, m_desktop, &output, m_viewOnly, m_log);
    ...
    sysLogMessage.format(_T("The client %s #%d has been disconnected for the reason: %s"),
...
void RfbClient::getViewPortInfo(const Dimension* fbDimension, Rect* resultRect, bool* shareApp, Region* shareAppRegion)
{
  AutoLock al(&m_viewPortMutex);
