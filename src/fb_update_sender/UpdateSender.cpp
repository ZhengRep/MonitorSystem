...
UpdateSender::UpdateSender(RfbCodeRegistrator* codeRegtor, UpdateRequestListener* updReqListener, SenderControlInformationInterface* senderControlInformation, RfbOutputGate* output, int id, Desktop* desktop, LogWriter* log)
  : m_updReqListener(updReqListener),
    ...
    m_cursorUpdates(log)
{
  m_updateKeeper = new UpdateKeeper(&unmove(Rect()));
...
void UpdateSender::init(const Dimension* viewPortDimension, const PixelFormat* pf)
{
  ...
  m_updateKeeper->setBorderRect(&unmove(viewPortDimension->getRect()));
}
...
void UpdateSender::readUpdateRequest(RfbInputGate* io)
{
  ...
  m_log->info(_T("update requested (%d, %d, %dx%d, incremental = %d) by client (client #%d)"),reqRect.left, reqRect.top,
    reqRect.getWidth(), reqRect.getHeight(), (int)incremental, m_id);
  ...
  while (!isTerminating()) {
    ...
    if (!isTerminating()) {
      try {
        ...
        m_log->interror(_T("The update sender thread caught an error and will be terminated: %s"), e.getMessage());
        Thread::terminate();
      }
    }
  }
...
void UpdateSender::sendUpdate()
{
  ...
  m_log->debug(_T("Time between request and a point after extractReqRegions (in milliseconds): %u"), (unsigned int)(DateTime::now() - reqTimePoint).getTime());
  m_log->debug(_T("A request has been made, continuing"));
  m_log->debug(_T("The incremental region has %d rectangles"), (int)requestedIncrReg.getCount());
  m_log->debug(_T("The full region has %d rectangles"), (int)requestedFullReg.getCount());
  ...
  if (dimensionChanged || viewPortChanged) {
    ...
    m_updateKeeper->setBorderRect(&unmove(lastViewPortDim.getRect()));
    updCont.changedRegion.crop(&unmove(lastViewPortDim.getRect()));
    // Dazzle changedRegion
    updCont.changedRegion.addRect(&unmove(lastViewPortDim.getRect()));
  ...
  // Send updates
  if (updCont.screenSizeChanged || (!requestedFullReg.isEmpty() && !encodeOptions.desktopSizeEnabled())) {
    m_log->debug(_T("Screen size changed or full region requested"));
    if (encodeOptions.desktopSizeEnabled()) {
      m_log->debug(_T("Desktop resize is enabled, sending NewFBSize %dx%d"), lastViewPortDim.width, lastViewPortDim.height);
      sendNewFBSize(&lastViewPortDim);
    } else {
      m_log->debug(_T("Desktop resize is disabled, sending blank screen")); 
      sendFbInClientDim(&encodeOptions, frameBuffer, &clientDim, &unmove(frameBuffer->getPixelFormat()));
    }
    ...
    m_cursorUpdates.update(&encodeOptions, &updCont, !requestedFullReg.isEmpty(), &viewPort, shareOnlyApp, &shareAppRegion, frameBuffer, &cursorShape);
    ...
    m_log->debug(_T("Number of normal rectangles before splitting: %d"), changedRegion.getCount());
    ...
    if (losslessEnabled && !losslessRegion.isEmpty()) {
      m_log->debug(_T("Number of lossless rectangles before splitting: %d"), losslessRegion.getCount());
    ...
    if (numTotalRects != 0) {
      ...
      m_log->debug(_T("Time between request and a point before send and coding (in milliseconds): %u"), (unsigned int)(DateTime::now() - reqTimePoint).getTime());
...
void UpdateSender::updateFrameBuffer(UpdateContainer* updCont, bool shareOnlyApp, const Region* prevSharedRegion, const Region* shareAppRegion)
{
  ...
  changedAndCopyRgns.addRect(&unmove(m_cursorUpdates.getBackgroundRect()));
  {
    AutoLock al(&m_reqRectLocMut);
    changedAndCopyRgns.add(&m_requestedFullReg);
  }

  updCont->screenSizeChanged = !m_desktop->updateExternalFrameBuffer(&m_frameBuffer, &changedAndCopyRgns, &viewPort) || updCont->screenSizeChanged;
}
...
void UpdateSender::sendFbInClientDim(const EncodeOptions* encodeOptions, const FrameBuffer* fb, const Dimension* dim, const PixelFormat* pf)
{
  ...
  Region region(&unmove(dim->getRect()));
...
void UpdateSender::sendCursorShapeUpdate(const PixelFormat* fmt, const CursorShape* cursorShape)
{
  ...
  sendRectHeader(hotSpot.x, hotSpot.y, dim.width, dim.height, PseudoEncDefs::RICH_CURSOR);

  FrameBuffer fbConverted;
  fbConverted.setProperties(&dim, fmt);
  m_pixelConverter.convert(&unmove(dim.getRect()), &fbConverted, cursorShape->getPixels());
