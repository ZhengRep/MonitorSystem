...
{
  {
    ...
    if (m_instance != 0) {
      throw Exception(_T("The only one instance of WindowsInputBlocker is allowed"));
    }
    m_instance = this;
  }
  resume();
...
void WindowsInputBlocker::execute()
{
  ...
  try {
    ...
    m_log->error(_T("The WindowsInputBlocker thread failed with error: %s"), e.getMessage());
  }
