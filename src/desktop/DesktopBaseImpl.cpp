...
void DesktopBaseImpl::getCurrentUserInfo(StringStorage* desktopName, StringStorage* userName)
{
...
void DesktopBaseImpl::getDisplayNumberCoords(Rect* rect, unsigned char dispNumber)
{
  _ASSERT(m_userInput != 0);
  _ASSERT(m_extDeskTermListener != 0);
...
bool DesktopBaseImpl::updateExternalFrameBuffer(FrameBuffer* fb, const Region* region, const Rect* viewPort)
{
...
void DesktopBaseImpl::sendUpdate()
{
  ...
  try {
    ...
    m_log->info(_T("WinDesktop::sendUpdate() failed with error:%s"), e.getMessage());
    m_extDeskTermListener->onAbnormalDesktopTerminate();
  }

  if (!updCont.isEmpty() || !m_fullReqRegion.isEmpty()) {
    m_log->debug(_T("UpdateContainer is not empty. Updates will be given to all."));
