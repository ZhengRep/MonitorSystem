...
bool GrabOptimizator::grab(const Region* grabRegion, ScreenDriver* grabber)
{
  ...
  if (getOptimizationAvailable(grabber)) {
    ...
    double boundsRectT = avgWholeT * boundsRectS / m_wholeS; 
    ...
    if (boundsRectT <= estimatedFragTime) {
      ...
      m_log->debug(_T("Bounds rectangle grab has been preferred: bounds rectangle time = %d, estimated fragment time = %d, estimated bounds rect time = %d"),
        (int)realBoundsRectTime, (int)estimatedFragTime, (int)boundsRectT);
    } else {
      __int64 realFragTime = grabFragments(&rects, grabber);
      m_log->debug(_T("Fragment grab has been preferred: fragment time = %d, estimated fragment time = %d, estimated bounds rect time = %d"),
...
{
  m_log->debug(_T("GrabOptimizator::m_wholeS : %d"), m_wholeS);

  StringStorage value;
  StringStorage statString;
  for (std::list<double>::iterator iter = m_wholeTElements.begin(); iter != m_wholeTElements.end(); iter++) {
  ...
  m_log->debug(_T("GrabOptimizator::m_wholeT average: %.2f; GrabOptimizator::m_wholeTSum: %.2f; GrabOptimizator::m_wholeTElements: %s"),
    avgWholeT, m_wholeTSum, statString.getString());

  statString.setString(_T(""));
  for (std::list<double>::iterator iter = m_gElements.begin(); iter != m_gElements.end(); iter++) {
    value.format(_T(" %.2f;"), *iter);
    statString.appendString(value.getString());
  }
  double avgG = m_gElements.size() != 0 ? m_gSum / m_gElements.size() : 0;
  m_log->debug(_T("GrabOptimizator::m_g average: %.2f; GrabOptimizator::m_gSum: %.2f; GrabOptimizator::m_gElements: %s;"),
    avgG, m_gSum, statString.getString());
}
...
__int64 GrabOptimizator::grabOneRect(const Rect* rect, ScreenDriver* grabber)
{
  int rectS = rect->area();
  _ASSERT(rectS != 0);
  LARGE_INTEGER timeBegin, timeEnd;
...
__int64 GrabOptimizator::grabFragments(const std::vector<Rect>* rects, ScreenDriver* grabber)
{
  LARGE_INTEGER timeBegin, timeEnd;
  bool timerResult1 = QueryPerformanceCounter(&timeBegin) != 0;
