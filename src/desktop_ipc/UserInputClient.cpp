...
void UserInputClient::getCurrentUserInfo(StringStorage* desktopName, StringStorage* userName)
{
  AutoLock al(m_forwGate);
  try {
    // Send request
...
void UserInputClient::getDisplayNumberCoords(Rect* rect, unsigned char dispNumber)
{
  AutoLock al(m_forwGate);
  bool success = false;
  do {
...
void UserInputClient::onRequest(UINT8 reqCode, BlockingGate* backGate)
{
  ...
  default:
    StringStorage errMess;
    errMess.format(_T("Unknown %d protocol code received from a pipe UserInputServer"), (int)reqCode);
