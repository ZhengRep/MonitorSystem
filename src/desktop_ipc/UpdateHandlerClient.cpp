  ...
  try {
    ...
    if (updCont.screenSizeChanged) {
      ...
      if (!newPf.isEqualTo(&oldPf) || !newDim.isEqualTo(&oldDim)) {
        m_log->info(_T("UpdateHandlerClient: new screen size: %dx%d"), newDim.width, newDim.height);
        m_log->info(_T("UpdateHandlerClient: new pixel format: %d, %d, %d, %d, %d, %d, %d, %d"),
          (int)newPf.bigEndian,
...
void UpdateHandlerClient::onRequest(UINT8 reqCode, BlockingGate* backGate)
{
  ...
  default:
    ...
    errMess.format(_T("Unknown %d protocol code received from a pipe update detector"), (int)reqCode);
