...
void DesktopServerProto::checkPixelFormat(const PixelFormat* pf)
{
  ...
  if (pf->bitsPerPixel != 16 && pf->bitsPerPixel != 32) {
    errMess.format(_T("Wrong value of bits per pixel (%d)"), (int)pf->bitsPerPixel);
    throw Exception(errMess.getString());
  }
  if (pf->colorDepth > pf->bitsPerPixel) {
    errMess.format(_T("Wrong value (color depth (%d) > bits per pixel (%d))"), (int)pf->colorDepth, (int)pf->bitsPerPixel);
    throw Exception(errMess.getString());
  }
}
...
{
  StringStorage errMess;
  if (abs(rect->left) > 32000 || abs(rect->top) > 32000 ||
    abs(rect->right) > 32000 || abs(rect->bottom) > 32000 ||
    !rect->isValid()) {
    errMess.format(_T("Wrong rectangle (%d, %d, %d, %d)"), rect->left, rect->top, rect->right, rect->bottom);
    throw Exception(errMess.getString());
  }
}
...
void DesktopServerProto::checkDimension(const Dimension* dim)
{
  ...
  if (abs(dim->width) > 64000 || abs(dim->height) > 64000) {
    errMess.format(_T("Wrong dimension (%dx%d)"), dim->width, dim->height);
    throw Exception(errMess.getString());
  }
}
