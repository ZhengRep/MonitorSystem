...
void UpdateHandlerServer::extractReply(BlockingGate* backGate)
{
  ...
  if (updCont.cursorShapeChanged) {
    ...
    sendDimension(&unmove(curSh->getDimension()), backGate);
    sendPoint(&unmove(curSh->getHotSpot()), backGate);
...
{
  const FrameBuffer* fb = m_updateHandler->getFrameBuffer();
  sendPixelFormat(&unmove(fb->getPixelFormat()), backGate);
  sendDimension(&unmove(fb->getDimension()), backGate);
}
...
void UpdateHandlerServer::serverInit(BlockingGate* backGate)
{
  ...
  readFrameBuffer(&fb, &unmove(dim.getRect()), backGate);
